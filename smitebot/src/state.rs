...
pub struct CampaignState {
    ...
    /// ISO 8601 timestamp when the campaign stopped.
...
pub enum Status {
    /// Campaign state created, nothing spawned yet.
    Created,
...
pub struct RunnerState {
    /// Runner index (0 = main, 1..N = secondary).
    pub id: u16,
...
mod tests {
    ...
    fn save_round_trips_through_json() {
        ...
        assert_eq!(loaded.runners[0].name, "main");
    ...
    fn status_serializes_as_lowercase() {
        ...
        let json = serde_json::to_string(&Status::Failed).unwrap();
