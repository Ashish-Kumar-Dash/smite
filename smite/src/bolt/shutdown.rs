...
mod tests {
    ...
    fn p2sh() -> Vec<u8> {
        let mut spk = vec![OP_HASH160.to_u8(), OP_PUSHBYTES_20.to_u8()];
        spk.extend_from_slice(&[0x22; 20]);
        spk.push(OP_EQUAL.to_u8());
        spk
    }

    fn p2wpkh() -> Vec<u8> {
        let mut spk = vec![OP_PUSHBYTES_0.to_u8(), OP_PUSHBYTES_20.to_u8()];
        spk.extend_from_slice(&[0x33; 20]);
        spk
    }

    fn p2wsh() -> Vec<u8> {
        let mut spk = vec![OP_PUSHBYTES_0.to_u8(), OP_PUSHBYTES_32.to_u8()];
        spk.extend_from_slice(&[0x44; 32]);
        spk
    }
    ...
    #[test]
    fn is_acceptable_shutdown_script_accepts_legacy() {
        // Receivers MAY accept P2PKH/P2SH, but they are non-standard.
        for spk in [p2pkh(), p2sh()] {
            assert!(!is_standard_shutdown_script(&spk, &all_shutdown_features()));
            assert!(is_acceptable_shutdown_script(&spk, &no_features()));
            assert!(is_acceptable_shutdown_script(
                &spk,
                &all_shutdown_features()
            ));
        }
    ...
    #[test]
    fn is_acceptable_shutdown_script_accepts_witness_v0() {
        // Witness v0 forms are always acceptable.
        for spk in [p2wpkh(), p2wsh()] {
            assert!(is_acceptable_shutdown_script(&spk, &no_features()));
            assert!(is_acceptable_shutdown_script(
                &spk,
    ...
    #[test]
    fn is_acceptable_shutdown_script_accepts_anysegwit() {
        // anysegwit is still feature-gated.
        assert!(!is_acceptable_shutdown_script(&anysegwit(), &no_features()));
        assert!(is_acceptable_shutdown_script(
            &anysegwit(),
            &with_anysegwit()
        ));
    ...
    #[test]
    fn is_acceptable_shutdown_script_accepts_simple_close() {
        // simple_close is still feature-gated.
        assert!(!is_acceptable_shutdown_script(
            &simple_close(),
            &no_features()
        ));
    ...
    #[test]
    fn is_acceptable_shutdown_script_rejects_other() {
